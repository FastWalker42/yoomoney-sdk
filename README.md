...
| да | `true` | игнорируется | `op.amount >= amount` (точное сравнение) |
| да | `false` | `"sender"` (по умолчанию) | `op.amount >= amount` (получатель должен получить ровно `amount`) |
| да | `false` | `"receiver"` | `op.amount >= amount * 0.97` (допуск до 3% — комиссия вычтется из `amount`) |
...
| `feePayer` | `"sender" \| "receiver"` | — | Кто платит комиссию. Если задан, SDK корректирует `sum`: `"sender"` → `sum * 1.03`, `"receiver"` → `sum` без изменений |
...
| `"sender"` | `sum * 1.03` | Отправитель платит `sum + 3%`, получатель получает `sum` |
...
```typescript
// Хотим получить 500 ₽. SDK автоматически сделает sum = 500 * 1.03 = 515.
const link = generatePaymentLink({
  receiver: "4100118425529732",
  sum: 500,
  feePayer: "sender",
...
// Проверяем, что получатель получил ровно 500 ₽
...
// Принимаем платежи, где получатель получил >= 500 * 0.97 = 485 ₽
const result = await client.checkPaymentByLabel("order-42", {
  amount: 500,
  feePayer: "receiver",
});
